...
def display(del_path,ipath,opath):
    with open(del_path, "r") as file:
        lines = file.readlines()
    
    base_image = cv2.imread(ipath)  # Replace with your existing image
    ...
    shutil.copyfile(ipath, opath)  # Make a copy of the original image
    cv2.imwrite(opath, copied_image) 
    
    cv2.imshow("Delaunay Triangulation", copied_image)

display("Feature_Matching_DT\Delaunay_Triangulation\output_vertices_edges_a.txt","Feature_Matching_DT\Fast_Detection\input1.jpeg","Feature_Matching_DT\Fast_Detection\output1.jpeg")
display("Feature_Matching_DT\Delaunay_Triangulation\output_vertices_edges_b.txt","Feature_Matching_DT\Fast_Detection\input2.jpeg","Feature_Matching_DT\Fast_Detection\output2.jpeg")
...
base_image = cv2.imread("Feature_Matching_DT\Fast_Detection\input1.jpeg")  # Replace with your existing image
height, width = base_image.shape[:2]
vertices2 = []
for line in lines2:
    if line.startswith("Vertex"):
